...
test('perTenantJwksKeyProvider fetches the tenant-specific JWKS URL and verifies against it', async () => {
  ...
  const fetchMock = mock.fn(async (input: string | URL) => {
    assert.equal(String(input), 'https://admin.acme.authstar.app/.well-known/jwks.json')
  ...
  try {
    ...
    const getKey = perTenantJwksKeyProvider('authstar.app')
